"""Show AR-model predictions on raw real frames (canonical median as bg).
Same layout as viz_real_predictions_on_raw but using the AR rollout instead
of PCA decode."""
...
from sim2real.scripts.eval_ar import load_ckpt, sample_attachments, rollout_batch
...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        for gt in row["gt"]:
            ...
            # Scale rollout from model canvas back to canonical 256 for display
            ...
            ax.text(gt_canon[0, 1] + 3, gt_canon[0, 0] - 3,
                    f"d={best_dist:.1f}", fontsize=7, color=color,
    ...
    fig.suptitle("V8 AR predictions on real (canonical bg), yellow=closest rollout, "
                 "green GT=hit, red GT=miss", fontsize=10)
    fig.tight_layout()
