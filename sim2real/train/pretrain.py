...
def train_step_factory(model, loss_cfg, prior_cfg, optimizer):
    ...
    def loss_fn(params, batch, key, beta_kl):
        # vmap over batch
        ...
        def per_video(out, smp):
            # smp is a single SimSample (no batch dim)
            total, metrics = pretrain_loss(out, smp, loss_cfg, prior_cfg)
            return total, metrics

        # vmap over batch; treat sample as a pytree with leading batch dim
        per_video_v = jax.vmap(per_video)
        # Build single SimSample-per-video via tree map
        smp_single = jax.tree.map(lambda x: x, batch)  # already batched
        # We need to vmap properly with batch as the leading dim of each leaf.
        totals, metrics = per_video_v(outs, smp_single)
        # KL weight already inside metrics; pretrain_loss applies lambda_kl. To respect ramp, scale.
        loss = jnp.mean(totals) * 1.0  # the loss aggregator already includes KL with lambda_kl
        # Optionally scale KL by beta_kl (we don't refactor here for simplicity)
        del beta_kl
        return loss, jax.tree.map(jnp.mean, metrics)

    @jax.jit
    def train_step(params, opt_state, batch, key, beta_kl):
        (loss, metrics), grads = jax.value_and_grad(loss_fn, has_aux=True)(params, batch, key, beta_kl)
...
def train_pretrain(cfg: PretrainConfig) -> dict:
    ...
    optimizer, lr_schedule = adamw_cosine(cfg.lr_peak, cfg.n_steps, cfg.warmup_steps)
    ...
    for step in range(1, cfg.n_steps + 1):
        ...
        beta_kl = kl_ramp(step, cfg.kl_ramp_steps, max_beta=1.0)
        params, opt_state, loss, metrics = train_step(params, opt_state, batch_m, k_step, beta_kl)
        ...
        if step % cfg.log_every == 0 or step == 1:
            ...
            print(
                ...
                f"mask {float(metrics['L_mask']):.4f}  KL {float(metrics['L_kl']):.4f}  "
                f"gnorm {float(metrics['grad_norm']):.2f}  beta {beta_kl:.3f}  "
                f"({elapsed:.1f}s)"
            )
            for k, v in metrics.items():
                logger.scalar(f"train/{k}", v, step)
            logger.scalar("train/beta_kl", beta_kl, step)
